...
test_timeout_monitor(_Config) ->
  ...
  timer:sleep(5500),
  {messages, [{test, 1, Pid}]} = erlang:process_info(self(), messages),
  timer:sleep(5500),
  {messages, [{test, 1, Pid}, {test, 2, Pid}]} = erlang:process_info(self(), messages),
  c:flush(),
...
test_timeout_monitor_2(_Config) ->
  ...
  timer:sleep(5500),
  {messages, [{test, 1, Pid}, {test, 2, Pid}, {test, 3, Pid}]} = erlang:process_info(self(), messages),
  c:flush(),
  gen_server:cast(Pid, test2),
  timer:sleep(5500),
  {messages, [{test, 4, Pid}]} = erlang:process_info(self(), messages),
  ignore = gen_server:call(Pid, test3),
  timer:sleep(5500),
  {messages, [{test, 4, Pid}, {test, 5, Pid}]} = erlang:process_info(self(), messages),
  c:flush(),
  ok = timeout_monitor:stop_monitor(Pid),
  ok.